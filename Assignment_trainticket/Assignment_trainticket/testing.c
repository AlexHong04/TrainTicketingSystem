...
void staff_maintain_menu(Member members[], staffdetail admin[])//?????check ????member????
{
	int memberCount = 0;
...
//this is home page for member information module(??)
void memberMenu(Member members[], int memberCount)
{
	int memberOption;
...
int readData(scheduleInfo northScheduleData[], scheduleInfo southScheduleData[]) {
	...
	while (fread(&allSchedule[count], sizeof(scheduleInfo), 1, NSFile)) {
		if (allSchedule[count].trainID.trainId[1] == 'N') {
			northScheduleData[scheduleNCount] = allSchedule[count];
			scheduleNCount++;
		}
		else if (allSchedule[count].trainID.trainId[1] == 'S') {
			southScheduleData[scheduleSCount] = allSchedule[count];
			scheduleSCount++;
		}
		count++;
	}
